...
class SoRMainClient(commands.Bot):
    ...
    def __register_commands(self):
        self.create_sor_game = self.command(
            name="sor",
            aliases=[
                "schwarz",
                "rot",
        ...
        self.source = self.command(name="source", aliases=["code", "sourcecode", "github"], pass_context=True)(
            self.source_code
        )

    async def on_ready(self):
        guilds = discord.utils.get(self.guilds)
        self.logger.info(f"{self.user.display_name} connected in [{guilds}]")
    ...
    async def source_code(self, ctx: commands.Context):
        await ctx.reply(
            "The source code for this bot is available at: https://github.com/CaptainJack42/schwarz_oder_rot_bot .\nFeel free to submit feature requests or pull requests there.",
            mention_author=True,
        )
